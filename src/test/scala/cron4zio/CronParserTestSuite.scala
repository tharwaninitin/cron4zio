...
object CronParserTestSuite {
  val desc: CronDescriptor = CronDescriptor.instance()
  ...
  val spec: Spec[Any, Any] =
    suite("Cron Parser")(
      ...
      test("parse should parse cron correctly but should fail to generate next run") {
        ...
        assertZIO(
          getNextRunDuration(parse(cron).get).foldZIO(ex => ZIO.succeed(ex.getMessage), _ => ZIO.succeed("ok"))
      ...
      test("parse should parse cron correctly: A run frequency of 11:00 PM every weekday night") {
        val cron = "0 0 23 ? * MON-FRI"
        println(desc.describe(parse(cron).get))
        assertTrue(parse(cron).isSuccess)
      },
      test("parse should parse cron correctly: A run frequency of 11:00 PM every Monday night") {
        assertTrue(parse("0 0 23 ? * MON").map(cron => desc.describe(cron)) == Success("at 23:00 at Monday day"))
      },
      ...
      test("parse should return Error when incorrect cron '0 */2 * ' provided") {
        assertTrue(parse("0 */2 * ").failed.get.getMessage == "Cron expression contains 3 parts but we expect one of [6, 7]")
      },
